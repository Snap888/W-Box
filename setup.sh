...
show_progress "Распаковка" "⏳ Извлечение wow64.tar.gx..." 2
...
if [ -f "$link_target" ]; then
    ...
    dialog --msgbox "🚫     Целевой файл не существует.\n
    ...
    BASE_DIR="/storage/emulated/0"
            while true; do
                ...
                if [ "$folder" = "0" ]; then
                    ...
                    selected_item=$(dialog --stdout --menu "Войдите в папку с файлами Mobox Menu и Widget. Нажмите < Ok >\n
─────────────────────────────────────\n
$name\n
─────────────────────────────────────\n
" 25 41 20 "${items_list[@]}")
...
if (dialog --title "❗️ Подтверждение" --yesno "
...
sed -i "5s|.*|DIR=$WEG_TXT|" "$DIR_DEFLF"
    ...
    if [ -d "$PREFIX/glibc-wow64" ] || [ -d "$PREFIX/glibc-x86" ] || [ -d "$PREFIX/glibc" ]; then
        choice=$(dialog --title "💡 Mobox box86 и WoW64" --menu "Выберите действие:" 13 38 3 \
...
" Midnight Commander (mc)" "" \
        ...
        choice=$(dialog --title "💡 Mobox box86 и WoW64" --menu "Выберите действие:" 12 38 3 \
" Установить Mobox box86 и WoW64" "" \
" Установить Mobox box86" "" \
" Установить Mobox WoW64" "" \
" Выбрать директорию с файлами" "" \
...
show_help() {
    dialog --title "📘 Справка" --msgbox "Это меню предназначено для управления пакетами WINE.\n\n
Вы можете:\n
...
if [ ${#menu_items[@]} -eq 0 ]; then
    dialog --title "🚫 Ошибка" --msgbox "Список пуст. Добавьте хотя бы один \n
...
folder_number=$(dialog --title "🍷 Выберите WINE для удаления" --menu "Будет удалена вся информация находящаяся в этом WINE" $menu_heightwinD 38 10 "${menu_items[@]}" 3>&1 1>&2 2>&3)
case $? in
    1) show_main_menu
    return ;; 
    255) show_main_menu
...
if [ "$WINE_ST" != "$(basename "$folder")" ]; then
dialog --title "❗️Подтверждение" --yesno "
...
select_folder() {
    ...
    if [ ${#menu_items[@]} -eq 0 ]; then
        dialog --title "🚫 Ошибка" --msgbox "В списке нет доступных WINE.\n
        Пожалуйста,\n
Добавьте файл WINE в список." 7 32
    ...
    folder_number=$(dialog --title "🍷 Выберите WINE для активации" --menu "" $menu_height_winE 38 10 "${menu_items[@]}" 3>&1 1>&2 2>&3)
    case $? in
        1) show_main_menu
        return ;;
        255) show_main_menu
    ...
    if [ "$WINE_ST" = "$folder_name" ]; then
        ...
        dialog --title "🔑 Активация" --msgbox "Данный WINE уже активен" 6 27
    else
        if [ ! -f "$path" ]; then
            touch "$path" || exit 1
        fi
...
while true; do
    ...
    archive_number=$(dialog --title "🍷 Импорт WINE в список" --menu "Выберите WINE для импорта:" "$menu_height_win" 35 10 "${archives[@]}" 3>&1 1>&2 2>&3)
    case $? in
        1)  show_main_menu
        return ;;
        255) show_main_menu
...
show_main_menu() {
WINE_ST="$( [ -f "$PREFIX/glibc/opt/conf/wine_path.conf" ] && head -n 1 "$PREFIX/glibc/opt/conf/wine_path.conf" | cut -c32- || echo "")"
    action=$(dialog --title "🍷 Управление WINE" --menu "Активный WINE:\n
$WINE_ST" 12 30 5 \
...
other_menu() {
choice=$(dialog --title "🛠 Дополнительно" --menu "Выберите опцию:" 13 42 3 \
...
" Информация" "" 2>&1 >/dev/tty)
                case $choice in
" Восстановить библиотеки") dialog --yesno "Восстановление всех библиотек\n         Продолжить?" 6 33
                    if [ $? -eq 0 ]; then
                    tar -xzf "$BASE_DIR/Mobox86_64/prefix.tar.gz" -C $PREFIX
                    fi
                    ;;
...
choice=$(dialog --title "Язык иеню Mobox" --menu "" 8 30 2 \
...
" Midnight Commander (mc)")
         mc
            ;;
" Информация") dialog --msgbox "💡 Информация \n\n
...
if [ $WINE_START_86 = 0 ]; then
	dialog --title "Установка WINE-mono" \
       --yesno "\n
...
;;
			...
			if [ -d "$wine" ]; then
    			...
    			dialog --title "🚫 Ошибка" --msgbox "Директория $wine не найдена." 5 40
			fi
			;;
            5) 
...
tar -xzf "$BASE_DIR/Mobox86_64/x86.tar.gz" -C "$temp_dir" x86
        ...
        exit
                ...
                other_menu
                ;;
                7.)  dialog --title "❗️Подтверждение" --yesno "Вы уверены, что хотите удалить \n
         Mobox box86? \n
...
сохранить все данные, т к. они \n
  буду утрачены безвозвратно." 9 35
                ...
                ;;
                7)  
                dialog --title "❗️Подтверждение" --yesno "Вы уверены, что хотите удалить \n
         Mobox WoW64? \n
Рекомендуем перед удалением \n
сохранить все данные, т к. они \n
  буду утрачены безвозвратно." 9 35
...
if [ -e $PREFIX/glibc/$WINE_ST/.wine/.update-timestamp ]; then
    ...
    options64+=(7. "Удалить Mobox box86")
      ...
      menu_height=$((11 + ${#options64[@]}/2))
choice=$(dialog --title "WEG™⚡️Mobox Menu v5.1" --menu "$dialog_text_64" "$menu_height" 41 "${#options64[@]}" "${options64[@]}" 2>&1 >/dev/tty)
    else
if [ -e $PREFIX/glibc/$WINE_ST/.wine/.update-timestamp ]; then
...
options86=(
    ...
    menu_height=$((11 + ${#options86[@]}/2))
choice=$(dialog --title "WEG™⚡️Mobox Menu v5.1" --menu "$dialog_text_86" "$menu_height" 41 "${#options86[@]}" "${options86[@]}" 2>&1 >/dev/tty)
fi
elif [ "$versioo_mob" = "WOW" ]; then
if [ -e $PREFIX/glibc/$WINE_ST/.wine/.update-timestamp ]; then
    NAME_MENU_64="Запуск Mobox WoW64"
...
choice=$(dialog --title "WEG™⚡️Mobox Menu v5.1" --menu "$dialog_text_64" 17 41 "${#options_64[@]}" "${options_64[@]}" 2>&1 >/dev/tty)
elif [ "$versioo_mob" = "86" ]; then
if [ -e $PREFIX/glibc/$WINE_ST/.wine/.update-timestamp ]; then
    NAME_MENU_86="Запуск Mobox box86"
    WINE_START_86=1
...
choice=$(dialog --title "WEG™⚡️Mobox Menu v5.1" --menu "$dialog_text_86" 17 41 "${#options_86[@]}" "${options_86[@]}" 2>&1 >/dev/tty)
fi
        case $? in
            1) break ;;
            255) continue ;;
...
[ -d "$temp_dir" ] && rm -rf "$temp_dir"
mkdir -p "$temp_dir"
show_progress "Распаковка файла" "⏳ Mobox WoW64.tar.gz..." 2
tar -xzf "$BASE_DIR/Mobox86_64/wow64.tar.gz" -C "$temp_dir"
show_progress "Распаковка файла" "⏳ Mobox box86.tar.gz..." 2
tar -xzf "$BASE_DIR/Mobox86_64/x86.tar.gz" -C "$temp_dir"
